...
st.set_page_config(page_title="SODEXAM - Gestion Intégrée Cloud", layout="wide", page_icon="🌧️")

# RÉFÉRENTIEL DES STATIONS (Spatialisation pour Surfer 13)
COORDS_STATIONS = {
...
def charger_releves():
    return conn.read(worksheet="relevés")

def charger_utilisateurs():
    return conn.read(worksheet="utilisateurs")

# --- 3. FONCTION ENVOI EMAIL ---
def envoyer_email_archive(destinataire, fichier_zip):
    try:
        ...
        msg['From'] = expediteur
        msg['To'] = destinataire
        msg['Subject'] = f"SODEXAM - ARCHIVE CLIMATIQUE : {datetime.now().strftime('%d/%m/%Y')}"
        
        corps = "Veuillez trouver ci-joint l'archive des relevés pluviométriques."
...
if not st.session_state.connecte:
    ...
    with st.container(border=True):
        ...
        if st.button("Se connecter", use_container_width=True):
            try:
                ...
                user = users[(users['username'].astype(str) == u) & (users['password'].astype(str) == p)]
                if not user.empty:
            ...
            except: st.error("Impossible de joindre la base utilisateur (Google Sheet).")
else:
    # --- BARRE LATÉRALE ---
    ...
    if st.session_state.user_role == "admin":
        menu.insert(1, "📊 Dashboard Admin (Email/Surfer)")
    ...
    if choix == "🌍 Carte Interactive":
        ...
        for v, coords in COORDS_STATIONS.items():
            ...
            if not df_total.empty:
                dv = df_total[df_total['Station'] == v]
                if not dv.empty:
                    last = dv.iloc[-1]
                    info = f"<b>{v}</b><br>Dernier relevé: {last['Pluie (mm)']} mm"
    ...
    # --- 📝 SAISIE RELEVÉ ---
        ...
        with st.form("form_saisie"):
            ...
            if st.form_submit_button("💾 Enregistrer sur le Cloud"):
                ...
                st.success("Donnée sauvegardée avec succès !")

    # --- 📊 DASHBOARD ADMIN ---
    elif choix == "📊 Dashboard Admin (Email/Surfer)":
        st.header("📊 Gestion Globale & Export")
        df_total = charger_releves()
        
        col1, col2 = st.columns(2)
        with col1:
            st.subheader("📬 Envoi Archive Email")
            dest = st.text_input("Email destinataire", "direction@sodexam.ci")
            if st.button("📤 Envoyer toutes les données par Email"):
                df_total.to_csv("backup_sodexam.csv", index=False)
                with zipfile.ZipFile("archive.zip", 'w') as z: z.write("backup_sodexam.csv")
                if envoyer_email_archive(dest, "archive.zip"):
                    st.success("Archive envoyée !")
                os.remove("backup_sodexam.csv"); os.remove("archive.zip")
    ...
    # --- 📈 ANALYSES ---
        ...
        if not df_t.empty:
            st.plotly_chart(px.bar(df_t, x='Station', y='Pluie (mm)', color='Station', title="Cumul Pluviométrique"))
